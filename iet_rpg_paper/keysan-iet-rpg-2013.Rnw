...
plot(g1pm_generator$Torque.kNm, g1pm_generator$Mass.t,pch=as.numeric(g1pm_generator$Manufacturer)+20, cex=1.5, col=colors[g1pm_generator$Manufacturer], bty="l", las=1,
     xlab="Torque (kNm)", ylab="Mass (t)")
grid()
linear_estimation<-lm(Mass.t ~ Torque.kNm, data=g1pm_generator)
abline(linear_estimation, col="gray", lwd=2, lty=2)
#Add Legend
legend(800,15, legend=levels(g1pm_generator$Manufacturer), col=colors,pch=21:22,
     bty="n", ncol=1, cex=0.8, pt.cex=1.5)
# display equation
#linear_coefficients <- round(coef(linear_estimation), 3) 
